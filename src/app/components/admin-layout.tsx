...
const navItems = [
  ...
  { icon: BarChart, label: "Leadership Board", path: "/admin/leadership" },
      ...
      <div className={`p-6 border-b border-slate-200 dark:border-slate-800 ${mobile ? "flex items-center justify-between" : ""}`}>
        <Link to="/" className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">Admin Portal</div>
          </div>
        </Link>
        {mobile && (
        ...
        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-slate-900 dark:text-white text-sm truncate">{user?.name || "Administrator"}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{(user as any)?.role || "System Admin"}</div>
            </div>
          </div>
        </div>
        <button
